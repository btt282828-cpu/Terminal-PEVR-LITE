...
def compute_cascada(df, rrg, flow=None):
    ...
    alto = [f["dmom"] for f in filas if f["orden"] in (1, 2)]
    bajo = [f["dmom"] for f in filas if f["orden"] in (3, 4)]
    spread = None
    ...
    cadena = [f for f in filas if f["orden"] in (1, 2, 3, 4)]
    if f5 and cadena:
